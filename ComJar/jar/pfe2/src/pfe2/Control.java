...
public class Control implements Initializable{
   ...
   public void initialize(URL arg0, ResourceBundle arg1) {
	...
	hamb = new HamburgerBackArrowBasicTransition(hamburger);
        ...
        int i=0;
        System.err.println("op "+op.getStartX());
        for (; i <=600; i++,x+=9) {
           listLine.add(new Line(0, x, 2000, x));
           if(j==0)
           {
               listLine.get(i).setStrokeWidth(2);
           }
           listLine.get(i).setId("gridX");
            listLine.get(i).setStroke(Paint.valueOf("c4bcbc"));
       ...
       for (i = 0; i < 800; i++,xy+=38.46) {          
           listLine1.add(new Line(xy, 0, xy, 1000));
           listLine1.get(i).toBack();
          listLine1.get(i).setId("gridY");
